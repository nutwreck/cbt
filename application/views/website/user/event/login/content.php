...
<head>
	...
	<title><?= @$event->name ?> | Zambert</title>
	...
	<div class="container">
		<div class="row py-5 mt-4 align-items-center">
			...
			<div class="col-md-5 mb-5 mb-md-0">
				<div id="carouselBanner" class="carousel slide" data-ride="carousel">
					<ol class="carousel-indicators">
						<?php
						if (count($banner) > 0) {
							foreach ($banner as $keyind => $vbanind) {
						?>
								<li data-target="#carouselBanner" data-slide-to="<?= $keyind ?>" class="<?= $keyind == 0 ? 'active' : '' ?>"></li>
						<?php
							}
						}
						?>
					...
					<div class="carousel-inner">
						<?php
						if (count($banner) > 0) {
							foreach ($banner as $keyban => $ban) {
						?>
								<div class="carousel-item <?= $keyban == 0 ? 'active' : '' ?>">
									<img src="<?= config_item('_dir_website') ?>lembaga/grandsbmptn/event/<?= $ban->image ?>" alt="carousel-banner-<?= $keyban ?>" class="img-fluid d-block w-100">
								</div>
						<?php
							}
						}
			...
			<div class="col-md-7 col-lg-6 ml-auto">
				<form id="form_login" name="form_login" action="<?php echo base_url(); ?>website/user/Event/submit_login" class="form-register" method="POST" enctype="multipart/form-data">
					<input type="hidden" id="csrf-hash-form" name="<?= $this->security->get_csrf_token_name(); ?>" value="<?= $this->security->get_csrf_hash(); ?>" style="display: none">
					<input type="hidden" id="prefix_url" name="prefix_url" value="<?= @$event->prefix_url ?>" style="display: none">
					<div class="row">
						...
						<div class="text-center w-100">
							<small class="text-muted font-weight-bold fs-18">Lupa Password? <a href="<?= base_url() ?>lupa-password/<?= @$event->prefix_url ?>" class="text-primary ml-2 fs-18">Klik Disini</a></small>
						</div>
				</form>
				<!-- Divider Text -->
				...
				<!-- Already Registered -->
				<div class="text-center w-100">
					<p class="text-muted font-weight-bold fs-18">Belum Punya Akun? <a href="<?= base_url() ?>event/<?= @$event->prefix_url ?>" class="text-primary ml-2 fs-18">Daftar</a></p>
				</div>
